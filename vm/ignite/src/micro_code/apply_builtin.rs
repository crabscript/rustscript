...
pub fn apply_builtin(t: &mut Thread, sym: &str, args: Vec<Value>) -> Result<()> {
    match sym {
        builtin::READ_LINE_SYM => {
            let input = builtin::read_line_impl()?;
            t.operand_stack.push(Value::String(input));
        }
        builtin::PRINT_SYM => {
            for arg in args {
        ...
        builtin::STRING_LEN_SYM => {
            ...
            t.operand_stack.push(Value::Int(len as i64));
        }
        builtin::MIN_SYM => {
        ...
        builtin::FLOAT_TO_INT_SYM => {
            ...
            t.operand_stack.push(float_to_int);
        }
        builtin::INT_TO_FLOAT_SYM => {
            ...
            t.operand_stack.push(int_to_float);
        }
        _ => {
...
mod tests {
    ...
    fn test_apply_builtin() -> Result<()> {
        ...
        let actual = t.operand_stack.pop().unwrap();
        ...
        apply_builtin(&mut t, sym, args)?;
        assert_eq!(Value::Int(42), t.operand_stack.pop().unwrap());

        let args: Vec<Value> = vec![Value::String("forty-two".to_string())];
        let result = apply_builtin(&mut t, sym, args);
        ...
        apply_builtin(&mut t, sym, args)?;
        assert_eq!(Value::Int(24), t.operand_stack.pop().unwrap());

        let args = vec![Value::Float(42.0), Value::Float(24.0)];
        apply_builtin(&mut t, sym, args)?;
        assert_eq!(Value::Float(24.0), t.operand_stack.pop().unwrap());
        ...
        let args = vec![Value::Float(42.0), Value::Float(24.0)];
        apply_builtin(&mut t, sym, args)?;
        assert_eq!(Value::Float(42.0), t.operand_stack.pop().unwrap());
        ...
        apply_builtin(&mut t, sym, args)?;
        assert_eq!(Value::Float(0.0_f64.cos()), t.operand_stack.pop().unwrap());
        ...
        let args = vec![Value::Float(0.0)];
        apply_builtin(&mut t, sym, args)?;
        assert_eq!(Value::Float(0.0), t.operand_stack.pop().unwrap());
