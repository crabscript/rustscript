...
impl TypeChecker {
    ...
    fn check_expr(expr: &Expr) -> Result<Type, TypeErrors> {
    ...
    fn check_decl(decl: &Decl) -> Result<(), TypeErrors> {
        match decl {
            Decl::LetStmt(stmt) => {
                let expr_type = TypeChecker::check_expr(&stmt.expr)?;
    ...
    pub fn type_check(self) -> Result<Type, TypeErrors> {
        ...
        for decl in self.program.decls.iter() {
            if let Err(mut decl_errs) = TypeChecker::check_decl(decl) {
